...
function WrtcChannel (sp, id) {
  ...
  function onData (data) {
    ...
    let deserialized
    try {
      deserialized = JSON.parse(data) // sometimes JSON come as UInt8Array; TODO: find better solution
    } catch(error) {
      //deserialized = BSON.deserialize(Buffer.from(data)) //parsing UInt8Array
      deserialized = msgpack.decode(Buffer.from(data))
    }
    self.emit('message', deserialized)
  }
...
WrtcChannel.prototype.send = function (data, binary = false) {
  var self = this
  if (self.destroyed) return

  //self._sp.send(binary ? BSON.serialize(data) : JSON.stringify(data))
  self._sp.send(binary ? msgpack.encode(data) : JSON.stringify(data))
}

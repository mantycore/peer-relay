...
WsChannel.prototype.send = function (data, binary = false) {
  var self = this
  if (self.destroyed) return
  if (self.ws.readyState === 2) return // readyState === CLOSING
  if (self.ws.readyState !== 1) throw new Error('WebSocket is not ready')

  //var str = binary ? BSON.serialize(data) : JSON.stringify(data)
  var str = binary ? msgpack.encode(data) : JSON.stringify(data)
  self.ws.send(str)
}

WsChannel.prototype._onMessage = async function (data) {
  var self = this
  if (self.destroyed) return

  var json = typeof data === 'string'
    ? JSON.parse(data)
    : typeof window !== 'undefined' && window.Blob && data instanceof Blob
    //? BSON.deserialize(Buffer.from(await new Response(data).arrayBuffer()))
    //: BSON.deserialize(data)
    ? msgpack.decode(Buffer.from(await new Response(data).arrayBuffer()))
    : msgpack.decode(data)
